...
export function Sidebar({ pathname, categories, pendingCategoryIds = [], clerkEnabled = false }: SidebarProps) {
  ...
  return (
    <aside className="flex w-full flex-1 min-h-0 flex-col rounded-[2rem] border border-border/70 bg-sidebar p-4 shadow-[0_20px_60px_-40px_rgba(15,23,42,0.45)]">
      ...
      <nav className="sidebar-client-scroll mt-6 min-h-0 flex-1 space-y-2 overflow-y-auto overflow-x-hidden pr-0.5">
      ...
      {/* ── Fixed bottom nav — always visible regardless of scroll ────────── */}
      <div className="mt-2 shrink-0 space-y-1 border-t border-border/40 pt-2">
        {[
